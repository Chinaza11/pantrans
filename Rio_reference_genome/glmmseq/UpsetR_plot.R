...
upset(fromList(listInput), keep.order = T, sets = c("Interaction", "Type", "Treatment"), mainbar.y.max = 1500, order.by="freq", mainbar.y.label="DEGs Intersections", sets.x.label="DEGs Per Effect", text.scale=c(1.5, 1.5, 1.25, 1.1, 2, 2), sets.bar.color=c('steelblue','purple','orange'), set_size.scale_max = 2000, shade.color = "gray50")
