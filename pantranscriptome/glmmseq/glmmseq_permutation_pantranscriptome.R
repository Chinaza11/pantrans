...
# =============================================================================
setwd("G:/My Drive/PhD/project/Iron_RNASeq_sorghum/data_analysis/data_and_result/pantranscriptome/glmmseq/") #------ in HPC environ: comment out
...
set.seed(1234) #setting random number generator
...
colnames <- colnames(combinations1)
...
process_column <- function(column) {
  modified <- sapply(column, function(cell) {
    parts <- unlist(strsplit(cell, ","))  # Split the cell by comma and space
...
# }
# 
# save(normalized_counts_list, file = "normalized_counts_list.Rdata")
# save(dispersions_list, file = "dispersions_list.Rdata")
...
table(subset_q_vals_type$comparison) #they only differ by 41 genes which means the permutation analysis is pretty valid. 1661 of the 1679 genes in the original grouping is significant in both.
#------------------------------------------------------------------------------
...
table(subset_q_vals_trt_type$comparison) #they only differ by 34 genes which means the permutation analysis is pretty valid. 192 of the 204 genes in the original grouping is significant in both.
#------------------------------------------------------------------------------
#calculating the permutation p-value
